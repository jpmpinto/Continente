...
export default function App() {
  ...
  const saveInvoiceToSupabase = async (artigos, total) => {
    console.log('Guardar no Supabase:', artigos, total);
    try {
      const { data: invoice, error: invoiceError } = await supabase
        .from('invoices')
        .insert([{ total, invoice_date: new Date().toISOString().slice(0, 10) }])
        .select()
        .single();

      if (invoiceError) {
        console.error('Erro ao inserir invoice:', invoiceError);
        throw invoiceError;
      }
